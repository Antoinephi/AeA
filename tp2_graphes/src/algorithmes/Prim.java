...
public class Prim {
	...
	public GraphImpl algo() {
		...
		/**
		 * On trie la liste des arrêtes par ordre croissant de valeur, 
		 * permet  par la suite d'accélérer le calcul
		 */
		...
		while (!sommetsMarques.isEmpty()) { // n-1 tours
			...
			for (Edge e : edgeList) { // m tours max
				/**
				 * Si la valeur de l'arrête courante est > à la val min enregristrée
				 * ça ne sert à rien d'aller plus loin, puisque la liste est triée.
				 */
				...
				//Un seul des sommets doit être marqué et la val de l'arrête < poidsMin
			...
			if (edgeMin != null) {
				...
				//On retire de la liste, moins de tours de boucles pour la suite
				edgeList.remove(edgeMin);
				//On marque les sommets de l'arrête en les retirant de la liste
